...
## 클라우드 서버 구축
안정적인 서비스 운영을 위해 AWS 서비스를 활용하여 서버를 구축한 프로젝트입니다.
...
CIDR 블록으로 사용할 IP 주소의 범위를 설정해서 두 개의 가용 영역에 VPC를 퍼블릭 서브넷과 프라이빗 서브넷을 구축했습니다. 인터넷 게이트웨이를 VPC에 배치해서 퍼블릭 서브넷의 라우팅 테이블은 인터넷 게이트웨이를 가리켜 외부 인터넷에서 트래픽을 주고 받을 수 있습니다. 보안 그룹과 네트워크ACL 설정으로 허용시킬 트래픽을 설정합니다. NAT Gateway 대신 NAT Instance를 사용함으로써 비용절감을 하고, 트래픽이 외부로 나갈 수 있지만 외부에서 접근할 수 없도록 보안을 강화했습니다.
...
처음, 퍼블릭 서브넷에 배치된 EC2 서비스 서버르르 배포해서 누구든지 서비스를 제공 받을 수 있게 구성했습니다. 하지만 퍼브릭 서브넷은 보안적으로 문제가 있었고, Bas Host를 도입하면서 서비스 서버를 프라이빗 서브넷으로 이전시켜 서비스 서버로 직접적인 접근이 불가능하도록 구성했습니다. 
...
RDS는 AWS에서 제공하는 관계형 데이터베이스 서비스입니다. AWS에서 운영 부분을 관리해줘서 중단 없이 스토리지를 확장하거나 백업, 마이그레이션을 쉽게 할 수 있습니다. 또한, Multi-AZ 배포, 읽기 전용 복제본, RDS Proxy를 사용햏서 데이터베이스의 가용성과 확장성을 확보했습니다.
